...
import { APPLICATION_FEE_INR } from "@/lib/franchiseFees";
export default function FranchisePaymentLink({ applicationId }: { applicationId: string }) {
  ...
  return <><button className="primary-button payment-link-button" onClick={continueToPayment} disabled={busy || !applicationId}>{busy ? "Creating secure payment link…" : `Pay ₹${APPLICATION_FEE_INR.toLocaleString("en-IN")} application fee`} <span>→</span></button>{message && <p className="payment-link-error" role="alert">{message}</p>}</>;
}
