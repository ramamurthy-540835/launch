...
export default function FranchiseRegistration({ initialArea = "", initialCity = "Chennai", opportunityId = "", onSuccess, redirectToPayment = false }: Props) {
  ...
  if (reference) return <section className="franchise-registration" id="register-franchise"><span className="kicker">APPLICATION RECEIVED</span><h2>Your franchise application is under review.</h2><p>Reference ID: <b>{reference}</b></p><p>Status: <b>Under review</b>. No payment is due unless LunchBox approves this application for payment.</p>{onSuccess && <button className="registration-done" onClick={onSuccess}>Done</button>}</section>;
