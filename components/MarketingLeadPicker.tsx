...
export default function MarketingLeadPicker({ leads, selectedIds, onChange, multiple = false, label = "Pipeline lead" }: Props) {
  ...
  return <div className={styles.picker}>
    <label>{label}<input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search saved schools or colleges" /></label>
    ...
    }) : <p>No matching saved schools or colleges. Add schools or colleges from Discover first.</p>}</div>
  </div>;
}
