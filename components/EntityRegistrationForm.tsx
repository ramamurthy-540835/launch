...
export default function EntityRegistrationForm({ entityType }: { entityType: EntityType }) {
  ...
  return <main className="school-search-page">
    <header className="topbar"><Link className="brand" href="/"><span className="brand-mark">L</span><span>Lunch<span>Box</span></span></Link><nav className="registration-mini-nav" aria-label="Registration types"><Link href="/schools/register">School</Link><Link className={entityType === "office" ? "active" : ""} href="/register/office">Office</Link><Link className={entityType === "company" ? "active" : ""} href="/register/company">Company</Link></nav></header>
    <section className="school-search-shell">
      <div className="school-search-heading"><span className="kicker">{copy.kicker}</span><h1>{copy.heading}</h1><p>{copy.description}</p></div>
      ...
      {selected && <form className="selected-school-panel entity-registration-panel" onSubmit={submit}>
        ...
        <div className="entity-form-section"><h3>{entityType === "office" ? "Workforce & meal details" : "Business & lunch opportunity"}</h3><div className="selected-school-fields"><label>Employee strength<input name="employee_strength" type="number" min="1" step="1" /></label><label>Expected lunch users<input name="expected_lunch_users" type="number" min="0" step="1" /></label><label>Existing food vendor<input name="existing_food_vendor" maxLength={160} /></label><label>Preferred lunch time<input name="preferred_meal_time" type="time" /></label><label>Meal price range<input name="meal_price_range" maxLength={80} placeholder="e.g. ₹80–₹120" /></label><label>Meal interest<select name="meal_interest" defaultValue=""><option value="">Select</option><option value="daily_lunch">Daily lunch</option><option value="subscription">Subscription</option><option value="events">Events / bulk meals</option><option value="exploring">Exploring</option></select></label>{entityType === "company" && <><label>Industry<input name="industry" maxLength={120} /></label><label>Company type<select name="company_type" defaultValue=""><option value="">Select</option><option>Private Limited</option><option>Limited Company</option><option>LLP</option><option>Partnership</option><option>Startup / SME</option></select></label></>}</div></div>
