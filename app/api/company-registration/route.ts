...
export async function POST(request: Request) {
  try {
    ...
    const result = await registerIndividualMealEnrollment("company", await request.json() as Record<string, unknown>, (task) => after(task));
    if (result.error) return NextResponse.json({ error: result.error }, { status: 400 });
    logInfo("meal_enrollment_received", { registration_type: "office_worker", location_entity_type: "company" });
