...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    if (!/^[A-Za-z0-9_-]{8,128}$/.test(id)) return NextResponse.json({ error: "Invalid application reference." }, { status: 400 });
    ...
    const reference = firestoreClient().collection("franchise_applications").doc(id);
    ...
    const callbackUrl = new URL(`/franchise/payment?applicationId=${encodeURIComponent(id)}`, request.url).toString();
    const link = await createFranchisePaymentLink({ applicationId: id, name: text(application.contactName) || text(application.name), email: text(application.email), phone: text(application.phone), callbackUrl });
