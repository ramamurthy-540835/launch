...
export async function POST(request: Request) {
  try {
    ...
    const { name, companyName, category, address, area, phone, website, contactName, email, latitude, longitude } = parsed.data;
    ...
    await reference.create({ name, companyName, category, address, area, city: "Chennai", phone, website, contactName, email, latitude: typeof latitude === "number" ? latitude : null, longitude: typeof longitude === "number" ? longitude : null, documentPaths: [], status: "pending", createdAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp() });
